...
?>
<aside class="w-56 bg-surface min-h-screen p-4 flex flex-col border-r border-bg">
    <div class="mb-6">
    ...
    <nav class="flex flex-col gap-1 flex-1">
        ...
        <a href="/admin/<?= $page === 'index' ? '' : $page . '.php' ?>"
           class="flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors <?= $current_page === $page ? 'bg-accent/20 text-accent font-medium' : 'text-muted hover:text-txt hover:bg-bg' ?>">
            <span><?= $item['icon'] ?></span>
            <?= $item['label'] ?>
        </a>
        <?php endforeach; ?>
